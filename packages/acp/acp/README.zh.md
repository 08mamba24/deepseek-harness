...
在 Loader 托管的应用中，`apply` 会捕获所属的 Loader 树，每次 `initialize` 都先等待它 settle——根级兄弟条目（尤其是 MCP 客户端）在第一个会话或提示词之前完成初始注册；无法 settle 的树会携带失败原因拒绝 `initialize`（fail-closed）。无 Loader 的应用行为不变。
...
| 方法 | 行为 |
|---|---|
| `initialize` | 在 Loader 托管的应用中先等待所属 Loader 树 settle（失败的树在协商前即被拒绝），然后协商受支持的版本。只有挂载持久附件存储，且配置的确切提供方／模型解析后明确支持图片输入时，才公布图片提示词能力；音频与嵌入上下文保持 false。不公布会话、编辑器、终端、文件系统或 MCP 能力。 |
